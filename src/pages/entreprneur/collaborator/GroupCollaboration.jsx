...
const EntrepreneurGroupedCollaborations = () => {
    ...
    const fetchAndGroupCollaborations = async () => {
        try {
            ...
            // Group collaborations by investor
            const grouped = collabs.reduce((acc, collab) => {
                const investorId = collab.investor_details.id;

                if (!acc[investorId]) {
                    acc[investorId] = {
                        investor: collab.investor_details,
                        collaborations: [],
                        projects: new Set(),
                        contracts: new Set(),
                        totalInvestment: 0
                ...
                acc[investorId].collaborations.push(collab);
                acc[investorId].projects.add(collab.project_name);
                if (collab.contract_details) {
                    acc[investorId].contracts.add(collab.contract_details.id);
                }
                if (collab.collaboration_type === 'financial' && collab.contract_details?.investment_amount) {
                    acc[investorId].totalInvestment += parseFloat(collab.contract_details.investment_amount);
                }
    ...
    const getFilteredInvestors = () => {
        return Object.entries(groupedCollabs).filter(([_, data]) => {
            ...
            if (searchQuery) {
                ...
                return (
                    data.investor.name.toLowerCase().includes(searchLower) ||
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Entrepreneurs List */}
            {getFilteredInvestors().map(([investorId, data]) => (
                <Card key={investorId} className="bg-white rounded-xl shadow-md overflow-hidden">
                    <div className="p-6">
                        <div className="flex justify-between items-start mb-4">
                            <div>
                                <h3 className="text-xl font-bold text-gray-900">{data.investor.name}</h3>
                        ...
                        <div className="space-y-3 mb-4">
                            <div className="flex items-center text-gray-600">
                                ...
                                <span className="text-sm">{data.investor.email}</span>
                            </div>
                            <div className="flex items-center text-gray-600">
                                <Phone className="h-4 w-4 mr-2 text-blue-500"/>
                                <span className="text-sm">{data.investor.phone}</span>
                            </div>
                        ...
                        <div className="mt-6 flex justify-between items-center">
                            ...
                            <button
                                onClick={() => setExpandedInvestor(
                                    expandedInvestor === investorId ? null : investorId
                                )}
                                className="flex items-center px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700"
                            >
                                {expandedInvestor === investorId ? (
                                    <>
...
export default EntrepreneurGroupedCollaborations;